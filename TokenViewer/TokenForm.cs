...
public partial class TokenForm : Form
{
    ...
    private void UpdateTokenData(ProcessTokenEntry? process)
    {
        ...
        if (!defdacl.NullAcl)
        {
            ...
            {
                UserGroup group = new(ace.Sid, GroupAttributes.None);
    ...
    private TokenForm(ProcessTokenEntry? process, NtToken token, string? text)
    {
        ...
        foreach (object v in Enum.GetValues(typeof(SaferLevelId)))
        {
            comboBoxSaferLevel.Items.Add(v);
        }

        comboBoxSaferLevel.SelectedItem = SaferLevelId.NormalUser;
    }
    ...
    private bool ShowTokenPermissions(TokenAccessRights access, bool throw_on_error)
    {
        ...
        if (result.IsSuccess)
        {
            using var dlg = new EditSecurityDescriptorDialog(result.Result, "Token", false);
            dlg.Show(Handle);
            return true;
        }
        return false;
    ...
    static List<TokenForm> _forms = new();
    ...
    static NtToken CreateProcessForToken(string cmdline, NtToken token, bool make_interactive)
    {
        ...
        if (make_interactive)
        {
            config.Desktop = @"WinSta0\Default";
            ...
            if (checkBoxUseWmi.Checked || checkBoxUseNetLogon.Checked)
            {
                ...
                else
                {
                    var config = new Win32ProcessConfig
                    {
                        ...
                        Credentials = new UserCredentials("abc", "abc", "abc"),
                        LogonFlags = CreateProcessLogonFlags.NetCredentialsOnly | CreateProcessLogonFlags.WithProfile
                    };
    ...
    private void btnComputeSafer_Click(object sender, EventArgs e)
    {
        try
        {
            OpenForm(Win32Security.GetTokenFromSaferLevel(_token,
                (SaferLevelId)comboBoxSaferLevel.SelectedItem, checkBoxSaferMakeInert.Checked), "Safer", false);                  
        }
        catch (Exception ex)
        {
            MessageBox.Show(this, ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
    ...
    {
        using CreateSandboxTokenForm form = new(_token);
