    ...
    protected override void ProcessRecord()
    {
        ...
        switch (ParameterSetName)
        {
            ...
            case "il_raw":
                ...
                break;
            case "package":
                sids = PackageName.Select(s => Win32Security.DerivePackageSidFromName(s));
                if (RestrictedPackageName != null)
                {
                    sids = sids.Select(s => Win32Security.DeriveRestrictedPackageSidFromSid(s, RestrictedPackageName));
                }
                if (AsCapability)
                {
