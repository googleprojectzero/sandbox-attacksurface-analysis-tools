...
public sealed class GetNtTokenCmdlet : PSCmdlet
{
    ...
    {
        return Win32Security.GetTokenFromClipboard(desired_access);
    }
    ...
    private NtToken GetS4UToken(TokenAccessRights desired_access)
    {
        using (GetTcbPrivilege())
        {
            using NtToken token = Win32Security.LsaLogonS4U(User, Domain, LogonType, AuthenticationPackage.NEGOSSP_NAME);
            if (desired_access == TokenAccessRights.MaximumAllowed)
            {
                return token.Duplicate();
            }
    ...
    private NtToken GetTicketToken(TokenAccessRights desired_access)
    {
        using (GetTcbPrivilege())
        {
            using NtToken token = Win32Security.LsaLogonTicket(LogonType, Ticket, KerbCred);
            if (desired_access == TokenAccessRights.MaximumAllowed)
            {
                return token.Duplicate();
            }
    ...
    {
        return Win32Security.GetAnonymousToken(desired_access);
    }
    ...
    {
        Sid package_sid = Win32Security.GetPackageSidFromName(PackageSid);
        if (!string.IsNullOrEmpty(RestrictedPackageName))
        {
            package_sid = Win32Security.DeriveRestrictedPackageSidFromSid(package_sid, RestrictedPackageName);
        }
    ...
    private NtToken GetSessionToken(TokenAccessRights desired_access, int session_id)
    {
        ...
        using var token = TerminalServicesUtils.GetSessionToken(session_id);
        if (desired_access == TokenAccessRights.MaximumAllowed)
        {
            return token.Duplicate();
        }
