...
namespace NtObjectManager
{
    ...
    /// </example>
    [Cmdlet(VerbsCommon.Get, "AccessibleProcessThread")]
    public class GetAccessibleProcessCmdlet : CommonAccessBaseCmdlet
    {
        ...
        internal override void RunAccessCheck(IEnumerable<TokenEntry> tokens)
        {
            ...
            {
                using (var procs = NtProcess.GetProcesses(ProcessAccessRights.ReadControl | ProcessAccessRights.QueryInformation).ToDisposableList())
                {
            ...
            else
            {
                ...
                foreach (var token in tokens)
                {
                    using (var processes = new DisposableList<NtProcess>())
                    {
                        ...
                        {
                            processes.AddRange(NtProcess.GetProcesses(ProcessAccessRights.MaximumAllowed | AccessRights));
                        }
                        foreach (NtProcess process in processes)
                        {
