...
public class NtDesktop : NtObjectWithDuplicate<NtDesktop, DesktopAccessRights>
{
    ...
    public static NtResult<NtDesktop> Open(ObjectAttributes object_attributes, CreateDesktopFlags flags, 
        DesktopAccessRights desired_access, bool throw_on_error)
    {
        return NtSystemCalls.NtUserOpenDesktop(object_attributes, flags, 
            desired_access).CreateWin32Result(throw_on_error, h => new NtDesktop(h));
    }
    ...
    public static NtDesktop Open(string desktop_name, NtObject root)
    {
        using ObjectAttributes obj_attributes
            = new(desktop_name, AttributeFlags.CaseInsensitive, root);
        ...
        bool throw_on_error)
    {
        ...
        return NtSystemCalls.NtUserCreateDesktopEx(object_attributes,
            string.IsNullOrEmpty(device) ? null : new UnicodeString(device),
            dev_mode, flags, desired_access, heap_size).CreateWin32Result(throw_on_error, h => new NtDesktop(h));
    }
    ...
    {
        using ObjectAttributes obj_attributes = new(desktop_name, AttributeFlags.CaseInsensitive, root);
    ...
    public static NtResult<NtDesktop> GetThreadDesktop(int thread_id, bool throw_on_error)
    {
        ...
        {
            return Win32Utils.CreateResultFromDosError<NtDesktop>(throw_on_error);
        }
    ...
    public NtStatus CloseDesktop(bool throw_on_error = true)
    {
        try
        {
            return NtSystemCalls.NtUserCloseDesktop(Handle).ToNtException(throw_on_error);
        }
        finally
        {
