...
public class NtWindowStation : NtObjectWithDuplicate<NtWindowStation, WindowStationAccessRights>
{
    ...
    {
        return NtSystemCalls.NtUserOpenWindowStation(object_attributes, 
            desired_access).CreateWin32Result(throw_on_error, h => new NtWindowStation(h));
    }
    ...
    {
        using ObjectAttributes obj_attr = new(winsta_name, AttributeFlags.CaseInsensitive, root);
        ...
        int language_id, int keyboard_locale, bool throw_on_error)
    {
        ...
        SafeBufferUtils.FillBuffer(buffer, 0);
        ...
        return NtSystemCalls.NtUserCreateWindowStation(object_attributes, desired_access, file.Result.Handle,
            layout_offset, nls_offset, buffer, 
            new UnicodeString($"{language_id:X08}"), keyboard_locale).CreateWin32Result(throw_on_error, h => new NtWindowStation(h));
    }
    ...
    public static IEnumerable<NtWindowStation> GetAccessibleWindowStations(WindowStationAccessRights desired_access)
    {
        ...
        foreach (string name in WindowStations)
        {
            ...
            using ObjectAttributes obj_attr = new(full_path, AttributeFlags.CaseInsensitive);
    ...
    public IEnumerable<NtDesktop> GetAccessibleDesktops(DesktopAccessRights desired_access)
    {
        ...
        {
            using ObjectAttributes obj_attr = new(desktop, AttributeFlags.CaseInsensitive, this);
    ...
    public NtStatus CloseWindowStation(bool throw_on_error = true)
    {
        try
        {
            return NtSystemCalls.NtUserCloseWindowStation(Handle).ToNtException(throw_on_error);
        }
        finally
        {
    ...
    {
        return NtSystemCalls.NtUserSetProcessWindowStation(Handle).ToNtException(throw_on_error);
    }
    ...
    public static NtResult<NtWindowStation> OpenCurrent(bool throw_on_error)
    {
        ...
        {
            return Win32Utils.CreateResultFromDosError<NtWindowStation>(throw_on_error);
        }
        return new NtWindowStation(new SafeKernelObjectHandle(handle, false)).CreateResult();
    }
