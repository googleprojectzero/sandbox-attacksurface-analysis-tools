...
public class SafeStructureInOutBuffer<T> : SafeHGlobalBuffer where T : new()
{
    ...
    public SafeHGlobalBuffer Data
    {
        get
        {
            ...
            int size = SafeBufferUtils.GetStructDataOffset<T>();
    ...
    /// </summary>
    public int DataOffset => SafeBufferUtils.GetStructDataOffset<T>();
    #endregion
    ...
    private static int GetTotalLength(int additional_size, bool add_struct_size)
    {
        if (add_struct_size)
        {
            int data_offset = SafeBufferUtils.GetIncludeField<T>()
                ? Marshal.SizeOf(typeof(T)) : SafeBufferUtils.GetStructDataOffset<T>();
