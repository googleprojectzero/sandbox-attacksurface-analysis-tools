...
public class SafeHGlobalBuffer : SafeBufferGeneric
{
    ...
    public SafeHGlobalBuffer GetBuffer(int offset, int size = int.MaxValue)
    {
        if (offset > Length)
            throw new ArgumentException("Offset is greater than length.");
        return new SafeHGlobalBuffer(DangerousGetHandle() + offset, Math.Min(Length - offset, size), false);
    }
}
