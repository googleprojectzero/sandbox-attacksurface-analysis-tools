...
using NtCoreLib.Process.Kernel;
...
public class NtProcess : NtObjectWithDuplicateAndInfo<NtProcess, ProcessAccessRights, ProcessInformationClass, ProcessInformationClass>
{
    ...
    private ProcessExtendedBasicInformation GetExtendedBasicInfo(bool get_cached)
    {
        if (_extended_info == null || !get_cached)
        {
            ...
            else
            {
                ...
                {
                    ProcessExtendedBasicInformation result = new()
                    {
    ...
    private static Enum ConvertPolicyToEnum(ProcessMitigationPolicy policy, int value)
    {
        ...
        {
            ProcessMitigationPolicy.ImageLoad => (ProcessMitigationImageLoadPolicy)value,
            ProcessMitigationPolicy.Signature => (ProcessMitigationBinarySignaturePolicy)value,
            ProcessMitigationPolicy.ControlFlowGuard => (ProcessMitigationControlFlowGuardPolicy)value,
            ProcessMitigationPolicy.DynamicCode => (ProcessMitigationDynamicCodePolicy)value,
            ProcessMitigationPolicy.ExtensionPointDisable => (ProcessMitigationExtensionPointDisablePolicy)value,
            ProcessMitigationPolicy.FontDisable => (ProcessMitigationFontDisablePolicy)value,
            ProcessMitigationPolicy.StrictHandleCheck => (ProcessMitigationStrictHandleCheckPolicy)value,
            ProcessMitigationPolicy.SystemCallDisable => (ProcessMitigationSystemCallDisablePolicy)value,
            ProcessMitigationPolicy.ChildProcess => (ProcessMitigationChildProcessPolicy)value,
            ProcessMitigationPolicy.PayloadRestriction => (ProcessMitigationPayloadRestrictionPolicy)value,
            ProcessMitigationPolicy.SystemCallFilter => (ProcessMitigationSystemCallFilterPolicy)value,
            ProcessMitigationPolicy.SideChannelIsolation => (ProcessMitigationSideChannelIsolationPolicy)value,
            ProcessMitigationPolicy.ASLR => (ProcessMitigationAslrPolicy)value,
            ProcessMitigationPolicy.UserShadowStack => (ProcessMitigationUserShadowStack)value,
            ProcessMitigationPolicy.RedirectionTrust => (ProcessMitigationRedirectionTrustPolicy)value,
            _ => (ProcessMitigationUnknownPolicy)value,
        };
    }
    ...
    private static NtProcessCreateResult Create(NtProcessCreateConfig config, string image_path, bool fork, bool throw_on_error)
    {
        ...
        SafeStructureInOutBuffer<SectionImageInformation> image_info = new();
        dispose.Add(ProcessAttribute.ImageInfo(image_info));
        SafeStructureInOutBuffer<ClientId> client_id = new();
    ...
    {
        List<NtThread> handles = new();
        if (IsAccessGranted(ProcessAccessRights.QueryInformation))
        {
            SafeKernelObjectHandle current_handle = new(IntPtr.Zero, false);
    ...
    public int GetRawMitigationPolicy(ProcessMitigationPolicy policy)
    {
        ...
        {
            NtStatus.STATUS_INVALID_PARAMETER or NtStatus.STATUS_NOT_SUPPORTED or NtStatus.STATUS_PROCESS_IS_TERMINATING => 0,
            _ => result.Result,
        };
    }
    ...
    {
        ProcessAccessToken proc_token = new()
        {
    ...
    public NtResult<long> AllocateMemory(long base_address,
        long region_size,
        MemoryAllocationType allocation_type, MemoryAllocationProtect protect, IEnumerable<MemoryExtendedParameter> extended_params,
        bool throw_on_error)
    {
        return NtVirtualMemory.AllocateMemory(Handle, base_address,
            region_size, allocation_type, protect, extended_params, throw_on_error);
    }
    ...
    public NtResult<long> AllocateMemory(long base_address,
        ...
        bool throw_on_error)
    {
        return AllocateMemory(base_address, region_size, allocation_type, 
            protect, Array.Empty<MemoryExtendedParameter>(), throw_on_error);
    }
    ...
    /// <returns>The address of the allocated region.</returns>
    /// <exception cref="NtException">Thrown on error.</exception>
    public long AllocateMemory(long base_address, long region_size,
        MemoryAllocationType allocation_type, MemoryAllocationProtect protect, 
        IEnumerable<MemoryExtendedParameter> extended_params)
    {
        return AllocateMemory(base_address, region_size, allocation_type, protect, extended_params, true).Result;
    }
    ...
    public IEnumerable<NtObject> GetHandleTableAsObjects(bool named_only, IEnumerable<string> type_names)
    {
        ...
        List<NtObject> objs = new();
        HashSet<string> types = new(type_names, StringComparer.OrdinalIgnoreCase);
    ...
    public override NtResult<object> QueryObject(ProcessInformationClass info_class, bool throw_on_error)
    {
        ...
        {
            ProcessInformationClass.ProcessBasicInformation => (NtResult<object>)Query<ProcessBasicInformation>(info_class, default, throw_on_error),
            ProcessInformationClass.ProcessIoCounters => (NtResult<object>)Query<IoCounters>(info_class, default, throw_on_error),
            ProcessInformationClass.ProcessTimes => (NtResult<object>)Query<KernelUserTimes>(info_class, default, throw_on_error),
            ProcessInformationClass.ProcessQuotaLimits => (NtResult<object>)Query<QuotaLimitsEx>(info_class, default, throw_on_error),
            ProcessInformationClass.ProcessVmCounters => (NtResult<object>)Query<VmCountersEx>(info_class, default, throw_on_error),
            ProcessInformationClass.ProcessCycleTime => (NtResult<object>)Query<ProcessCycleTimeInformation>(info_class, default, throw_on_error),
            ProcessInformationClass.ProcessProtectionInformation => (NtResult<object>)Query<PsProtection>(info_class, default, throw_on_error),
    ...
    /// </summary>
    public string[] CommandLineArguments => Win32ProcessUtils.ParseCommandLine(GetCommandLine(false).GetResultOrDefault(string.Empty));
    ...
    public ProcessDepStatus DepStatus
    {
        get
        {
            using SafeStructureInOutBuffer<uint> buffer = new();
            ProcessDepStatus ret = new();
    ...
    /// </summary>
    public NtProcessMitigations Mitigations => new(this);
    ...
    public static NtProcess Current => new(new SafeKernelObjectHandle(-1));
