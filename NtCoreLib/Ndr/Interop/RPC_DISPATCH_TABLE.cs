...
internal struct RPC_DISPATCH_TABLE : IConvertToNative<RPC_DISPATCH_TABLE>
{
    ...
    RPC_DISPATCH_TABLE IConvertToNative<RPC_DISPATCH_TABLE>.Read(IMemoryReader reader, IntPtr address, int index)
    {
        return reader.ReadStruct<RPC_DISPATCH_TABLE32>(address, index).Convert();
    }
}
