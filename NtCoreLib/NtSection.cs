...
public sealed class NtSection : NtObjectWithDuplicateAndInfo<NtSection, SectionAccessRights, SectionInformationClass, SectionInformationClass>
{
    ...
    {
        using ObjectAttributes obj_attr = new(path, AttributeFlags.CaseInsensitive, root);
    ...
    public static NtResult<NtSection> CreateEx(ObjectAttributes object_attributes, SectionAccessRights desired_access, LargeInteger size,
        MemoryAllocationProtect protection, SectionAttributes attributes, NtFile file, IEnumerable<SectionMemoryExtendedParameter> extended_parameters, bool throw_on_error)
    {
        using DisposableList list = new();
        var ext_params = extended_parameters?.Select(p => p.ToStruct(list))?.ToArray();
        return NtSystemCalls.NtCreateSectionEx(out SafeKernelObjectHandle section_handle, desired_access, object_attributes,
            ...
            ext_params, ext_params?.Length ?? 0).CreateResult(throw_on_error, () => new NtSection(section_handle));
    }
    ...
    public static NtSection CreateEx(ObjectAttributes object_attributes, SectionAccessRights desired_access, LargeInteger size,
        MemoryAllocationProtect protection, SectionAttributes attributes, NtFile file, IEnumerable<SectionMemoryExtendedParameter> extended_parameters)
    {
    ...
    {
        LargeInteger size = new(new_size);
