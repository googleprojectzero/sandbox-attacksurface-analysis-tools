...
namespace NtCoreLib.Security.Authorization.Policy;
...
public class CentralAccessPolicy
{
    ...
    {
        List<CentralAccessRule> capes = new();
    ...
    {
        Dictionary<int, CentralAccessRule> rules = new();
    ...
    {
        List<CentralAccessPolicy> policies = new();
    ...
    public static NtResult<CentralAccessPolicy[]> ParseFromLsa(bool throw_on_error)
    {
        ...
        List<CentralAccessPolicy> ret = new();
        using (capids)
        {
            ...
            {
                SafeHGlobalBuffer buffer = new(policy.CAPEs, policy.CAPECount * IntPtr.Size, false);
                IntPtr[] rule_entries = new IntPtr[policy.CAPECount];
                buffer.ReadArray(0, rule_entries, 0, policy.CAPECount);
                List<CentralAccessRule> rules = new();
