...
public sealed class SecurityDescriptor
{
    ...
    private static SecurityDescriptorSid ReadSid(NtProcess process, long address, bool defaulted)
    {
        ...
        Sid sid = new(process.ReadMemory(address, 8 + header.RidCount * 4, true));
    ...
    public Ace MandatoryLabel
    {
        ...
        set
        {
            ...
            Sacl ??= new Acl();
            Sacl.NullAcl = false;
            MandatoryLabelAce ace = value as MandatoryLabelAce ?? new MandatoryLabelAce(value.Flags, value.Mask.ToMandatoryLabelPolicy(), value.Sid);
            Sacl.Add(ace);
        }
    }
    ...
    public Ace FindSaclAce(AceType type, bool include_inherit_only)
    {
        if (Sacl != null && !Sacl.NullAcl)
        {
    ...
    public IEnumerable<Ace> FindAllSaclAce(AceType type, bool include_inherit_only)
    {
        if (Sacl != null && !Sacl.NullAcl)
        {
    ...
    {
        using (SafeHGlobalBuffer buffer = new(security_descriptor))
        {
    ...
    {
        SecurityDescriptor sd = new(type) { Container = container };
