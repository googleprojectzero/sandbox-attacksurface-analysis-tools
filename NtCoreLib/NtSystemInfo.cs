...
public static class NtSystemInfo
{
    ...
    private static readonly Dictionary<SystemInformationClass, object> _cached_info = new();
    private static readonly NtSystemInfoObject _system_info_object = new();
    ...
    {
        List<NtProcessInformation> ret = new();
    ...
    private static Lazy<OSVersionInfo> _os_version = new(GetOSVersion);

    private static OSVersionInfo GetOSVersion()
    {
        RTL_OSVERSIONINFOEXW info_ex = new();
    ...
    {
        int repeat_count = 10;
        Dictionary<int, string> image_paths = new()
        {
            [0] = "Idle",
            [4] = "System"
        };
    ...
    {
        SystemCodeIntegrityVerificationInformation info = new()
        {
    ...
    {
        SystemElamCertificateInformation info = new()
    ...
    {
        SystemCodeIntegrityCertificateInformation info = new()
        {
    ...
    public static NtResult<IEnumerable<ProcessModuleInformation>> GetKernelModules(bool throw_on_error)
    {
        using var buffer = QueryBuffer<RtlProcessModules>(SystemInformationClass.SystemModuleInformation, default, throw_on_error);
        if (!buffer.IsSuccess)
            return buffer.Cast<IEnumerable<ProcessModuleInformation>>();
        return ProcessModuleInformation.GetModules(buffer.Result).CreateResult();
    }
    ...
    /// </summary>
    /// <returns>The list of kernel modules.</returns>
    public static IEnumerable<ProcessModuleInformation> GetKernelModules()
    {
    ...
    public static IEnumerable<CodeIntegrityPolicy> CodeIntegrityFullPolicy
    {
        get
        {
            List<CodeIntegrityPolicy> policies = new();
            try
            {
                MemoryStream stm = new(QueryRawBytes(SystemInformationClass.SystemCodeIntegrityPoliciesFullInformation));
                if (stm.Length > 0)
                {
