...
namespace NtCoreLib.Utilities.Security.Authorization;
...
public sealed class ObjectTypeTree
{
    ...
    {
        ObjectTypeTree ret = new(object_type, name);
        AddNode(ret);
        return ret;
    }
    ...
    {
        List<ObjectTypeEntry> entries = new();
    ...
    public IEnumerable<ObjectTypeTree> Split(int max_entry)
    {
        ...
        List<ObjectTypeTree> ret = new();
        ObjectTypeTree new_tree = new(ObjectType, Name);
        ret.Add(new_tree);
    ...
    {
        entries.Add(new ObjectTypeEntry(ObjectType, level, Name ?? string.Empty));
        foreach (var node in Nodes)
        {
