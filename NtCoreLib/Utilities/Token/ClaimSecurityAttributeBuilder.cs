...
public class ClaimSecurityAttributeBuilder
{
    ...
    private Tuple<int[], byte[]> MarshalValues()
    {
        ...
        IEnumerable<int> offsets = ValueType switch
        {
            ClaimSecurityValueType.Int64 => Values.Cast<long>().Select(v => WriteValue(writer, writer.Write, v)),
            ClaimSecurityValueType.UInt64 => Values.Cast<ulong>().Select(v => WriteValue(writer, writer.Write, v)),
            ClaimSecurityValueType.Boolean => Values.Cast<bool>().Select(v => WriteValue(writer, writer.Write, v ? 1L : 0L)),
            ClaimSecurityValueType.String => Values.Cast<string>().Select(v => WriteString(writer, v)),
            ClaimSecurityValueType.OctetString => Values.Cast<byte[]>().Select(v => WriteOctet(writer, v)),
            ClaimSecurityValueType.Sid => Values.Cast<Sid>().Select(v => WriteOctet(writer, v.ToArray())),
            _ => throw new ArgumentException($"Unsupported claim type {ValueType}."),
        };
    ...
    public static ClaimSecurityAttributeBuilder Create(ClaimSecurityAttribute attribute)
    {
        ...
        {
            ClaimSecurityValueType.Boolean => Create(attribute.Name, attribute.Flags, attribute.Values.Cast<bool>().ToArray()),
            ClaimSecurityValueType.Fqbn => Create(attribute.Name, attribute.Flags, attribute.Values.Cast<ClaimSecurityAttributeFqbn>().ToArray()),
            ClaimSecurityValueType.Int64 => Create(attribute.Name, attribute.Flags, attribute.Values.Cast<long>().ToArray()),
            ClaimSecurityValueType.OctetString => Create(attribute.Name, attribute.Flags, attribute.Values.Cast<byte[]>().ToArray()),
            ClaimSecurityValueType.Sid => Create(attribute.Name, attribute.Flags, attribute.Values.Cast<Sid>().ToArray()),
            ClaimSecurityValueType.String => Create(attribute.Name, attribute.Flags, attribute.Values.Cast<string>().ToArray()),
            ClaimSecurityValueType.UInt64 => Create(attribute.Name, attribute.Flags, attribute.Values.Cast<ulong>().ToArray()),
            _ => throw new ArgumentException($"Unknown attribute value type {attribute.ValueType}"),
        };
    }
    #endregion
