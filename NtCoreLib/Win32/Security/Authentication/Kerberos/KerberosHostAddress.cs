...
public sealed class KerberosHostAddress : IDERObject
{
    ...
    public static KerberosHostAddress FromIPAddress(IPAddress address)
    {
        ...
        {
            AddressFamily.InterNetwork => new KerberosHostAddress(KerberosHostAddressType.IPv4, address.GetAddressBytes()),
            AddressFamily.InterNetworkV6 => new KerberosHostAddress(KerberosHostAddressType.IPv6, address.GetAddressBytes()),
            _ => throw new ArgumentException("Unknown address family."),
        };
    }
    ...
    internal static IReadOnlyList<KerberosHostAddress> ParseSequence(DERValue value)
    {
        if (!value.CheckSequence())
            throw new InvalidDataException();
        List<KerberosHostAddress> ret = new();
