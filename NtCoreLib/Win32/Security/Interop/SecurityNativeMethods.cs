...
using NtCoreLib.Win32.Security.Audit;
...
using NtCoreLib.Win32.Security.Sam;
using System;
...
internal static class SecurityNativeMethods
{
    ...
    [DllImport("samlib.dll", CharSet = CharSet.Unicode)]
    internal static extern NtStatus SamCreateAliasInDomain(
        SafeSamHandle DomainHandle,
        UnicodeString AccountName,
        SamAliasAccessRights DesiredAccess,
        out SafeSamHandle AliasHandle,
    ...
    [DllImport("samlib.dll", CharSet = CharSet.Unicode)]
    internal static extern NtStatus SamDeleteAlias(
        SafeSamHandle AliasHandle
    );
    ...
    [DllImport("sspicli.dll", CharSet = CharSet.Unicode)]
    internal static extern SecStatusCode SeciAllocateAndSetCallFlags(int flags, [MarshalAs(UnmanagedType.Bool)] out bool created);

    [DllImport("sspicli.dll", CharSet = CharSet.Unicode)]
    internal static extern void SeciFreeCallContext();
    ...
    [DllImport("advapi32.dll", SetLastError = true)]
    internal static extern bool SaferCreateLevel(SaferScopeId dwScopeId, SaferLevelId dwLevelId, SaferCreateLevelFlags OpenFlags, out SafeSaferLevelHandle pLevelHandle, IntPtr lpReserved);

    [DllImport("advapi32.dll", SetLastError = true)]
    internal static extern bool SaferCloseLevel(IntPtr hLevelHandle);

    [DllImport("advapi32.dll", SetLastError = true)]
    internal static extern bool SaferComputeTokenFromLevel(SafeSaferLevelHandle LevelHandle, SafeHandle InAccessToken,
        out SafeKernelObjectHandle OutAccessToken, SaferComputeTokenFlags dwFlags, IntPtr lpReserved);
    ...
    internal static extern bool ImageEnumerateCertificates(
        ...
        ImageCertificateType TypeFilter,
