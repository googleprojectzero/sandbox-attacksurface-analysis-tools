...
public sealed class SamDomain : SamObject
{
    ...
    {
        return account_type switch
        {
            SamUserAccountType.User => UserAccountControlFlags.NormalAccount,
            SamUserAccountType.Workstation => UserAccountControlFlags.WorkstationTrustAccount,
            SamUserAccountType.Server => UserAccountControlFlags.ServerTrustAccount,
            SamUserAccountType.InterDomain => UserAccountControlFlags.InterDomainTrustAccount,
            SamUserAccountType.TempDuplicate => UserAccountControlFlags.InterDomainTrustAccount,
            _ => throw new ArgumentException("Invalid account type.", nameof(account_type)),
        };
    }
    ...
    public NtResult<SamAlias> CreateAlias(string name, SamAliasAccessRights desired_access, bool throw_on_error)
    {
        return SecurityNativeMethods.SamCreateAliasInDomain(Handle, new UnicodeString(name), desired_access,
            out SafeSamHandle alias_handle, out uint rid).CreateResult(throw_on_error,
            () => new SamAlias(alias_handle, desired_access, ServerName, name, DomainId.CreateRelative(rid)));
    }
    ...
    /// <param name="desired_access">The desired access for the alias object.</param>
    /// <returns>The SAM alias object.</returns>
    public SamAlias CreateAlias(string name, SamAliasAccessRights desired_access = SamAliasAccessRights.MaximumAllowed)
    {
        return CreateAlias(name, desired_access, true).Result;
    }
