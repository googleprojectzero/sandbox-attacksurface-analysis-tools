...
public static class DirectoryServiceUtils
{
    ...
    private static readonly DomainDictionaryDict<Tuple<string, string>, DirectoryEntry> _root_entries = new();
    private static readonly DomainDictionaryDict<Guid, DirectoryServiceSchemaObject> _schema_class = new();
    private static readonly DomainDictionaryDict<string, DirectoryServiceSchemaObject> _schema_class_by_name 
        = new(StringComparer.OrdinalIgnoreCase);
    private static readonly DomainDictionaryDict<Guid, DirectoryServiceExtendedRight> _extended_rights = new();
    ...
    private static readonly DomainDictionaryDict<Guid, List<DirectoryServiceExtendedRight>> _extended_rights_by_applies_to 
        = new();
    private static readonly DomainDictionaryLazy _get_extended_rights = new(LoadExtendedRights);
    private static readonly DomainDictionaryLazy _get_schema_classes = new(LoadSchemaClasses);
    private static readonly DirectoryServiceExtendedRight _default_propset = new(string.Empty, string.Empty,
    ...
    private static DirectoryEntry GetRootEntry(string domain, string prefix, string context)
    {
        ...
        {
            DirectoryEntry entry = new(ConstructLdapUrl(domain, "RootDSE"));
            string path;
    ...
    {
        DirectorySearcher ds = new(root_object, filter, properties)
        {
            SearchScope = scope
        };
        return ds.FindOne();
    ...
    private static DirectoryServiceSchemaObject ConvertToSchemaClass(string domain, Guid? schema_id, SearchResult result)
    {
        ...
        if (class_names == null || class_names.Length < 1)
            return null;
            ...
            case "classschema":
                {
                    ...
                    List <DirectoryServiceSchemaClassAttribute> attrs = new();
                    ...
                    List<DirectoryServiceReferenceClass> aux_classes = new();
                    ...
                    List<DirectoryServiceReferenceClass> superior_classes = new();
    ...
    {
        List<DirectoryServiceExtendedRight> ret = new();
        try
        {
    ...
    {
        List<DirectoryServiceSchemaAttribute> ret = new();
        try
        {
            DirectoryEntry root_entry = GetRootEntry(domain, string.Empty, kSchemaNamingContext);
    ...
    {
        List<DirectoryServiceSchemaClass> ret = new();
