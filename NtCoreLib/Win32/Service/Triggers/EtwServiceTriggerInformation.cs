...
public sealed class EtwServiceTriggerInformation : ServiceTriggerInformation
{
    ...
    {
        using var reg = EventTraceUtils.Register(SubType);
        reg.Write();
    }

    private protected override string GetSubTypeDescription()
    {
        return $"{base.GetSubTypeDescription()} {EventTraceUtils.GetProviderName(SubType) ?? SubType.ToString("B")}";
    }

    internal EtwServiceTriggerInformation(SERVICE_TRIGGER trigger)
        : base(trigger)
    {
        _security_descriptor = new Lazy<SecurityDescriptor>(() => EventTraceUtils.QueryTraceSecurity(SubType, false).GetResultOrDefault());
    }
}
