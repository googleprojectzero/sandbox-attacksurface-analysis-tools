...
public sealed class ServiceConfig
{
    ...
    private static RegistryKey? OpenKeySafe(RegistryKey rootKey, string path)
    {
        try
        {
            return rootKey.OpenSubKey(path, RegistryRights.QueryValues);
        }
    ...
    internal ServiceConfig(string? machine_name, string name,
        ...
        SafeStructureInOutBuffer<QUERY_SERVICE_CONFIG>? config, bool delayed_auto_start)
    {
        ...
        if (!string.IsNullOrEmpty(MachineName))
            return;
        ImagePath = Win32ProcessUtils.GetImagePathFromCommandLine(BinaryPathName);
        ...
        if (key is not null)
        {
            ...
            if (!string.IsNullOrEmpty(ServiceDll))
            {
                string[] args = Win32ProcessUtils.ParseCommandLine(BinaryPathName);
    ...
    internal ServiceConfig(string? machine_name, string name)
        : this(machine_name, name, new ServiceTriggerInformation[0], ServiceSidType.None,
