...
public sealed class AppContainerProfile : IDisposable
{
    ...
    private AppContainerProfile(string name, Sid sid, IEnumerable<Sid>? capabilities,
        string? display_name, string? description, bool restricted)
    {
        Name = name;
        Sid = sid;
    ...
    private AppContainerProfile(string name)
        : this(name, Win32Security.DerivePackageSidFromName(name), 
            ...
            bool throw_on_error)
    {
        ...
        NtStatus status = NativeMethods.CreateAppContainerProfile(appcontainer_name, display_name, description,
            caps.Length > 0 ? caps : null, caps.Length, out SafeSidBufferHandle sid);
        if (status == Win32Error.ERROR_ALREADY_EXISTS.MapDosErrorToStatus())
        {
        ...
        return status.CreateResult(throw_on_error, () =>
            {
                using (sid)
                {
                    return new AppContainerProfile(appcontainer_name, sid.ToSid(),
                        capabilities, display_name, description, false);
                }
            });
    }
    ...
    {
        var sid = Win32Security.DerivePackageSidFromName(appcontainer_name, throw_on_error);
        if (!sid.IsSuccess)
        {
            return sid.Cast<AppContainerProfile>();
        }
        return new AppContainerProfile(appcontainer_name, sid.Result, null, null, null, false).CreateResult();
    }
    ...
    {
        return NativeMethods.DeleteAppContainerProfile(appcontainer_name).ToNtException(throw_on_error);
    }
    ...
    {
        var error = NativeMethods.NetworkIsolationEnumAppContainers(NETISO_FLAG.NONE, out int count, out IntPtr acs);
        ...
        {
            if (acs != IntPtr.Zero)
                NativeMethods.NetworkIsolationFreeAppContainers(acs);
        }
    }
    ...
    {
        using var result = Win32Security.CreateAppContainerToken(null, Sid, Array.Empty<Sid>(), throw_on_error);
        if (!result.IsSuccess)
            return result.Cast<NtKey>();
        using var imp = result.Result.Impersonate(SecurityImpersonationLevel.Impersonation);
        return NativeMethods.GetAppContainerRegistryLocation(desired_access, out SafeKernelObjectHandle key)
    ...
    public AppContainerProfile CreateRestrictedProfile(string restricted_name)
    {
        if (string.IsNullOrEmpty(restricted_name))
        {
            throw new ArgumentException($"'{nameof(restricted_name)}' cannot be null or empty.", nameof(restricted_name));
        }

        if (_restricted)
            throw new ArgumentException("Profile is already restricted.");

        return new AppContainerProfile($"{Name}/{restricted_name}", 
            Win32Security.DeriveRestrictedPackageSidFromSid(Sid, restricted_name), 
            null, null, null, true);
    }
    #endregion
    ...
    public string Path
    {
        get
        {
            if (NativeMethods.GetAppContainerFolderPath(Sid.ToString(), out SafeCoTaskMemBuffer path).IsSuccess())
            {
                using (path)
                {
