...
public sealed class RpcProtocolTower
{
    ...
    /// </summary>
    public RpcSyntaxIdentifier? Interface => Floors[0].GetIdentifier();

    /// <summary>
    /// The RPC transfer syntax.
    /// </summary>
    public RpcSyntaxIdentifier? TransferSyntax => Floors[1].GetIdentifier();
    ...
    public static RpcProtocolTower CreateTower(RpcSyntaxIdentifier interface_id, RpcSyntaxIdentifier transfer_syntax, RpcStringBinding string_binding)
    {
        ...
        switch (RpcProtocolSequence.StringToId(string_binding.ProtocolSequence))
        {
            ...
            case RpcProtocolSequenceIdentifier.Container:
                return CreateHVSocketTower(interface_id, transfer_syntax, Guid.Parse(string_binding.Endpoint),
                    RpcHyperVClientTransport.ResolveVmId(string_binding.NetworkAddress));
            default:
    ...
    public static RpcProtocolTower CreateLrpcTower(RpcSyntaxIdentifier interface_id, RpcSyntaxIdentifier transfer_syntax, string port_name)
    {
    ...
    /// <returns>The created tower.</returns>
    public static RpcProtocolTower CreateTcpTower(RpcSyntaxIdentifier interface_id, RpcSyntaxIdentifier transfer_syntax, ushort port, IPAddress address)
    {
    ...
    /// <returns>The created tower.</returns>
    public static RpcProtocolTower CreateNamedPipeTower(RpcSyntaxIdentifier interface_id, RpcSyntaxIdentifier transfer_syntax, string pipe_name, string hostname)
    {
        return CreateTower(interface_id, transfer_syntax, RpcProtocolIdentifier.ConnectionOrientatedProtocol,
    ...
    /// <returns>The created tower.</returns>
    public static RpcProtocolTower CreateHVSocketTower(RpcSyntaxIdentifier interface_id, RpcSyntaxIdentifier transfer_syntax, Guid? service_id, Guid? vm_id)
    {
        return CreateTower(interface_id, transfer_syntax, RpcProtocolIdentifier.ConnectionOrientatedProtocol,
    ...
    private static RpcProtocolTower CreateTower(RpcSyntaxIdentifier interface_id, RpcSyntaxIdentifier transfer_syntax,
            RpcProtocolIdentifier id, params RpcProtocolTowerFloor[] additional_floors)
    {
        List<RpcProtocolTowerFloor> floors = new()
        {
            new RpcProtocolTowerFloor(interface_id),
            new RpcProtocolTowerFloor(transfer_syntax),
            new RpcProtocolTowerFloor(id, 0)
        };
    ...
    private static string GetEndpoint(RpcProtocolSequenceIdentifier id, byte[] endpoint)
    {
        return id switch
        {
            RpcProtocolSequenceIdentifier.Tcp or RpcProtocolSequenceIdentifier.Udp or RpcProtocolSequenceIdentifier.Http => BitConverter.ToUInt16(endpoint, 0).SwapEndian().ToString(),
            _ => BinaryEncoding.Instance.GetString(endpoint).TrimEnd('\0'),
        };
    }
