...
public sealed class RpcTransportSecurityContext
{
    ...
    internal int AuthDataLength => TransportSecurity.AuthenticationLevel switch
    {
        RpcAuthenticationLevel.PacketIntegrity => AuthContext.MaxSignatureSize,
        RpcAuthenticationLevel.PacketPrivacy => AuthContext.SecurityTrailerSize,
        _ => 0,
    };
        ...
        RpcTransportSecurity transport_security, int context_id)
    {
        ...
        if (transport_security.AuthenticationLevel == RpcAuthenticationLevel.Default)
        {
            transport_security.AuthenticationLevel = RpcAuthenticationLevel.None;
        }
    ...
    internal byte[] ProtectPDU(byte[] header, ref byte[] stub_data, int auth_padding_length, int send_sequence_no)
    {
        ...
        else if (TransportSecurity.AuthenticationLevel == RpcAuthenticationLevel.PacketPrivacy)
        {
            ...
            RpcTransportUtils.DumpBuffer(_client_transport.TraceFlags, RpcTransportTraceFlags.Security, "Send Encrypted Data", stub_data);
        }

        if (signature.Length > 0)
        {
            RpcTransportUtils.DumpBuffer(_client_transport.TraceFlags, RpcTransportTraceFlags.Security, "Send Signature Data", signature);
        }
