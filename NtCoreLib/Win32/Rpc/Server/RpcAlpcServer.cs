...
/// </summary>
public sealed class RpcAlpcServer
{
    /// <summary>
    ...
    {
        using var transport = new RpcAlpcClientTransport(path, null, null);
    ...
    private static void GetAlpcServersInternal(NtProcess process, IEnumerable<NtHandle> handles, List<RpcAlpcServer> servers, bool ignore_com_interfaces)
    {
        ...
        {
            if (ignore_com_interfaces && handle.Name.IndexOf(@"\RPC Control\OLE", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                continue;
            }
            ...
            if (eps.IsSuccess)
            {
                RpcAlpcServer server = new(handle, process_name, eps.Result, ignore_com_interfaces);
                if (server.EndpointCount > 0)
                {
                    servers.Add(server);
                }
            }
