...
internal sealed class SafeRpcBindingHandle : SafeHandleZeroOrMinusOneIsInvalid
{
    ...
    {
        return NativeMethods.RpcBindingFree(ref handle) == 0;
    }
    ...
    {
        var status = NativeMethods.RpcBindingFromStringBinding(string_binding, out SafeRpcBindingHandle binding);
    ...
    {
        var status = NativeMethods.RpcStringBindingCompose(objuuid, protseq,
    ...
    public override string ToString()
    {
        ...
        {
            if (NativeMethods.RpcBindingToStringBinding(handle, out SafeRpcStringHandle str) == 0)
            {
                using (str)
                {
                    return str.ToString();
    ...
    public static SafeRpcBindingHandle Null => new(IntPtr.Zero, false);
}
