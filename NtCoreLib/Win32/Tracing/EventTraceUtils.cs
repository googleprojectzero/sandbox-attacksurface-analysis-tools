...
/// </summary>
public static class EventTraceUtils
{
    #region Private Members
    private static readonly Lazy<Dictionary<Guid, EventTraceProvider>> _providers = new(GetProvidersInternal);

    private static Dictionary<Guid, EventTraceProvider> GetProvidersInternal()
    {
        int retry_count = 10;
        int buffer_length = 1024;
        Dictionary<Guid, EventTraceProvider> providers = new();
        while (retry_count-- > 0)
        {
            using var buffer = new SafeStructureInOutBuffer<PROVIDER_ENUMERATION_INFO>(buffer_length, false);
            Win32Error error = NativeMethods.TdhEnumerateProviders(buffer, ref buffer_length);
    ...
    {
        int length = 0;
        Win32Error error = NativeMethods.EventAccessQuery(ref guid, SafeHGlobalBuffer.Null, ref length);
        ...
        error = NativeMethods.EventAccessQuery(ref guid, buffer, ref length);
        if (error != Win32Error.SUCCESS)
        {
            return error.CreateResultFromDosError<SecurityDescriptor>(throw_on_error);
        }
    ...
    public static NtStatus ControlTraceSecurity(Guid guid, EventSecurityOperation operation, Sid sid, TraceAccessRights access_mask, bool allow, bool throw_on_error)
    {
        ...
        return NativeMethods.EventAccessControl(ref guid, operation, buffer, access_mask, allow).ToNtException(throw_on_error);
    }
    ...
    {
        return NativeMethods.EventAccessRemove(ref guid).ToNtException(throw_on_error);
    }
    ...
    {
        return NativeMethods.EventRegister(ref guid, null, IntPtr.Zero, out long handle)
    ...
    public static NtResult<EventTraceLog> Start(string logfile, Guid session_guid, string session_name, bool throw_on_error)
    {
        ...
        return NativeMethods.StartTrace(out long handle, session_name,
            buffer).CreateWin32Result(throw_on_error, () => new EventTraceLog(handle, session_guid, session_name, buffer));
    }
    ...
    public static IEnumerable<Guid> GetTraceGuids()
    {
        ...
        while (true)
        {
            ...
            Win32Error error = NativeMethods.EnumerateTraceGuidsEx(TRACE_QUERY_INFO_CLASS.TraceGuidQueryList,
