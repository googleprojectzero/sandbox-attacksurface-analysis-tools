...
public static class FilterManagerUtils
{
    ...
    {
        return EnumFilter(b => NativeMethods.FilterFindFirst(FILTER_INFORMATION_CLASS.FilterAggregateStandardInformation, b, b.GetLength(), out _, out IntPtr handle).CreateResult(false, () => handle),
            (h, b) => NativeMethods.FilterFindNext(h, FILTER_INFORMATION_CLASS.FilterAggregateStandardInformation, b, b.GetLength(), out _),
            NativeMethods.FilterFindClose, (SafeStructureInOutBuffer<FILTER_AGGREGATE_STANDARD_INFORMATION> b) => b.Result.NextEntryOffset,
    ...
    {
        return EnumFilter(b => NativeMethods.FilterInstanceFindFirst(filter_name, INSTANCE_INFORMATION_CLASS.InstanceFullInformation, b, b.GetLength(), out _, out IntPtr handle).CreateResult(false, () => handle),
            (h, b) => NativeMethods.FilterInstanceFindNext(h, INSTANCE_INFORMATION_CLASS.InstanceFullInformation, b, b.GetLength(), out _),
            NativeMethods.FilterInstanceFindClose, (SafeStructureInOutBuffer<FILTER_INSTANCE_FULL_INFORMATION> b) => b.Result.NextEntryOffset,
    ...
    {
        return EnumFilter(b => NativeMethods.FilterVolumeInstanceFindFirst(volume_name, INSTANCE_INFORMATION_CLASS.InstanceFullInformation, b, b.GetLength(), out _, out IntPtr handle).CreateResult(false, () => handle),
            (h, b) => NativeMethods.FilterVolumeInstanceFindNext(h, INSTANCE_INFORMATION_CLASS.InstanceFullInformation, b, b.GetLength(), out _),
            NativeMethods.FilterVolumeInstanceFindClose, (SafeStructureInOutBuffer<FILTER_INSTANCE_FULL_INFORMATION> b) => b.Result.NextEntryOffset,
            (SafeStructureInOutBuffer<FILTER_INSTANCE_FULL_INFORMATION> b) => new FilterInstance(b));
    }
    ...
    {
        return EnumFilter(b => NativeMethods.FilterVolumeFindFirst(FILTER_VOLUME_INFORMATION_CLASS.FilterVolumeStandardInformation, b, b.GetLength(), out _, out IntPtr handle).CreateResult(false, () => handle),
            (h, b) => NativeMethods.FilterVolumeFindNext(h, FILTER_VOLUME_INFORMATION_CLASS.FilterVolumeStandardInformation, b, b.GetLength(), out _),
            NativeMethods.FilterVolumeInstanceFindClose, (SafeStructureInOutBuffer<FILTER_VOLUME_STANDARD_INFORMATION> b) => b.Result.NextEntryOffset,
    ...
    public static NtResult<string> Attach(string filter_name, string volume_name, string altitude, string instance_name, bool throw_on_error)
    {
        ...
        StringBuilder builder = new(INSTANCE_NAME_MAX_CHARS);
        if (string.IsNullOrEmpty(altitude))
        {
            return NativeMethods.FilterAttach(filter_name, volume_name,
                instance_name, builder.Capacity, builder).CreateResult(throw_on_error, builder.ToString);
        }
        return NativeMethods.FilterAttachAtAltitude(filter_name, volume_name, altitude,
                instance_name, builder.Capacity, builder).CreateResult(throw_on_error, builder.ToString);
    }
    ...
    {
        return NativeMethods.FilterDetach(filter_name, volume_name, instance_name).ToNtException(throw_on_error);
    }
        ...
        FindClose find_close, GetNextOffset<U> get_next, CreateObject<T, U> create) where U : struct
    {
        ...
        NtResult<IntPtr> handle = new(no_more_hresult, IntPtr.Zero);
        var list = new List<T>();
        try
        {
