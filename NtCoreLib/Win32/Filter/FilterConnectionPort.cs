...
public class FilterConnectionPort : NtFile
{
    ...
    {
        return NativeMethods.FilterConnectCommunicationPort(port_name, sync_handle ? FilterConnectFlags.FLT_PORT_FLAG_SYNC_HANDLE : 0,
    ...
    public NtResult<FilterConnectionPortMessage> GetMessage(int max_message_size, bool throw_on_error)
    {
        ...
        return NativeMethods.FilterGetMessage(Handle, buffer,
    ...
    {
        FILTER_REPLY_HEADER header = new()
        {
        ...
        return NativeMethods.FilterReplyMessage(Handle, buffer, buffer.Length).ToNtException(throw_on_error);
    }
    ...
    {
        return NativeMethods.FilterSendMessage(Handle, input ?? SafeHGlobalBuffer.Null,
