...
public static class DeviceUtils
{
    ...
    public static IEnumerable<string> GetDeviceInterfaceList(Guid interface_class_guid, string device_id, bool all_devices)
    {
        ...
        while (true)
        {
            var result = NativeMethods.CM_Get_Device_Interface_List_Size(out int length, ref interface_class_guid, device_id, flags);
            ...
            result = NativeMethods.CM_Get_Device_Interface_List(ref interface_class_guid, device_id, buffer, buffer.Length, flags);
            if (result == CrError.SUCCESS)
            {
    ...
    {
        NativeMethods.CM_Locate_DevNodeW(out int root, instance_id, 0).ToNtStatus().ToNtException();
    ...
    {
        NativeMethods.CM_Locate_DevNodeW(out int root, null, 0).ToNtStatus().ToNtException();
        Dictionary<int, DeviceTreeNode> nodes = new();
    ...
    {
        NativeMethods.CM_Locate_DevNodeW(out int dev_inst, instance_id, 0).ToNtStatus().ToNtException();
    ...
    internal static DeviceProperty[] GetDeviceProperties(int dev_inst)
    {
        ...
        NativeMethods.CM_Get_DevNode_Property_Keys(dev_inst, null, ref count, 0);
        if (count == 0)
        {
        ...
        if (NativeMethods.CM_Get_DevNode_Property_Keys(dev_inst, keys, ref count, 0) != CrError.SUCCESS)
        {
            return new DeviceProperty[0];
        }
    ...
    internal static IEnumerable<string> GetDeviceIdList(string filter, CmGetIdListFlags flags)
    {
        ...
        while (retry_count-- > 0)
        {
            NativeMethods.CM_Get_Device_ID_List_SizeW(out int length, filter, flags).ToNtStatus().ToNtException();
            using var buffer = new SafeHGlobalBuffer(length * 2);
            var result = NativeMethods.CM_Get_Device_ID_ListW(filter, buffer, length, flags);
            if (result == CrError.BUFFER_SMALL)
                continue;
    ...
    {
        List<string> ret = new();
        StringBuilder builder = new(NativeMethods.MAX_DEVICE_ID_LEN);
        int index = 0;
        int length = NativeMethods.MAX_DEVICE_ID_LEN;
        CrError result = NativeMethods.CM_Enumerate_EnumeratorsW(index++, builder, ref length, 0);
        while (result != CrError.NO_SUCH_VALUE)
        {
            ...
            length = NativeMethods.MAX_DEVICE_ID_LEN;
            result = NativeMethods.CM_Enumerate_EnumeratorsW(index++, builder, ref length, 0);
        }
        return ret;
    }
    ...
    internal static NtResult<string> GetClassString(Guid class_guid, bool interface_guid, DEVPROPKEY key, bool throw_on_error)
    {
        using var buffer = GetClassProperty(class_guid, interface_guid ? CmClassType.Interface : CmClassType.Installer,
                key, out DevicePropertyType type, throw_on_error);
        if (!buffer.IsSuccess)
    ...
    {
        int length = 4;
        var result = NativeMethods.CM_Get_Class_PropertyW(class_guid, key, out DevicePropertyType type, out int value, ref length,
        ...
        if (type != DevicePropertyType.UINT32)
    ...
    {
        int length = 16;
        var result = NativeMethods.CM_Get_Class_PropertyW(class_guid, key, out DevicePropertyType type, out Guid value, ref length,
            interface_guid ? CmClassType.Interface : CmClassType.Installer).ToNtStatus();
        if (!result.IsSuccess())
            return result.CreateResultFromError<Guid>(throw_on_error);
        if (type != DevicePropertyType.GUID)
    ...
    {
        int length = 0;
        NativeMethods.CM_Get_Class_Property_Keys(class_guid,
        ...
        if (NativeMethods.CM_Get_Class_Property_Keys(class_guid,
    ...
    {
        int length = 0;
        NativeMethods.CM_Get_Device_Interface_Property_KeysW(interface_instance,
            null, ref length, 0);
        if (length <= 0)
            return new DEVPROPKEY[0];

        DEVPROPKEY[] keys = new DEVPROPKEY[length];
        if (NativeMethods.CM_Get_Device_Interface_Property_KeysW(interface_instance,
    ...
    {
        if (NativeMethods.CM_Get_Device_ID_Size(out int length, devinst, 0) != CrError.SUCCESS)
            return string.Empty;
        StringBuilder builder = new(length);
        if (NativeMethods.CM_Get_Device_IDW(devinst, builder, length, 0) != CrError.SUCCESS)
            return string.Empty;
        builder.Length = length;
    ...
    {
        return NativeMethods.CM_Open_Class_KeyW(class_guid, null, desired_access, disposition,
    ...
    {
        DeviceProperty ret = new() { Name = DevicePropertyKeys.KeyToName(key), FmtId = key.fmtid, Pid = key.pid, Data = new byte[0] };
        using (var buffer = new SafeHGlobalBuffer(2000))
        {
            int length = buffer.Length;
            if (NativeMethods.CM_Get_DevNode_PropertyW(devinst, key, out DevicePropertyType type, buffer, ref length, 0) == CrError.SUCCESS)
            {
                ret.Type = type;
    ...
    internal static DeviceProperty GetProperty(Guid class_guid, bool interface_guid, DEVPROPKEY key)
    {
        DeviceProperty ret = new() { Name = DevicePropertyKeys.KeyToName(key), FmtId = key.fmtid, Pid = key.pid, Data = new byte[0] };
        using (var buffer = GetClassProperty(class_guid, interface_guid ? CmClassType.Interface : CmClassType.Installer,
            key, out DevicePropertyType type, false).GetResultOrDefault())
        {
            if (buffer != null)
            {
                ret.Type = type;
    ...
    internal static string[] GetClassStringList(Guid class_guid, bool interface_guid, DEVPROPKEY key)
    {
        ...
        if (prop.Type != DevicePropertyType.STRING_LIST)
            return new string[0];
    ...
    {
        DeviceProperty ret = new() { Name = DevicePropertyKeys.KeyToName(key), FmtId = key.fmtid, Pid = key.pid, Data = new byte[0] };
        using (var buffer = GetDeviceInterfaceProperty(link_path, key, out DevicePropertyType type, false).GetResultOrDefault())
        {
            if (buffer != null)
            {
                ret.Type = type;
    ...
    internal static string GetDeviceName(int devinst)
    {
        ...
        if (prop?.Type == DevicePropertyType.STRING)
    ...
    {
        return NativeMethods.CM_MapCrToWin32Err(error, Win32Error.ERROR_INVALID_PARAMETER).MapDosErrorToStatus();
    }
    ...
    private static IEnumerable<DeviceNode> GetDeviceList(OptionalGuid class_guid, string enumerator, DiGetClassFlags flags)
    {
        ...
        NativeMethods.CM_Locate_DevNodeW(out int root, null, 0).ToNtStatus().ToNtException();
        using var p = NativeMethods.SetupDiGetClassDevsW(class_guid, enumerator, IntPtr.Zero, flags);
        if (p.IsInvalid)
            Win32Utils.GetLastWin32Error().ToNtException();
        int index = 0;
        int size = Marshal.SizeOf(typeof(SP_DEVINFO_DATA));
        SP_DEVINFO_DATA dev_info = new() { cbSize = size };
        while (NativeMethods.SetupDiEnumDeviceInfo(p, index++, ref dev_info))
        {
    ...
    private static IEnumerable<Guid> EnumerateClasses(CmClassType flags)
    {
        ...
        while (true)
        {
            var result = NativeMethods.CM_Enumerate_Classes(index++, ref guid, flags);
    ...
    {
        int length = 0;
        var result = NativeMethods.CM_Get_Class_Registry_PropertyW(class_guid, property, out RegistryValueType reg_type,
        ...
        return NativeMethods.CM_Get_Class_Registry_PropertyW(class_guid, property, out reg_type,
            buffer, ref length, 0, IntPtr.Zero).ToNtStatus().CreateResult(throw_on_error, () => buffer.Detach());
    }

    private static NtResult<SafeHGlobalBuffer> GetClassProperty(Guid class_guid, CmClassType flags, DEVPROPKEY key, out DevicePropertyType type, bool throw_on_error)
    {
        int length = 0;
        var result = NativeMethods.CM_Get_Class_PropertyW(class_guid, key, out type, SafeHGlobalBuffer.Null, ref length, flags);
        if (result != CrError.BUFFER_SMALL)
        {
            return result.ToNtStatus().CreateResultFromError<SafeHGlobalBuffer>(throw_on_error);
        }
        ...
        using var buffer = new SafeHGlobalBuffer(length);
        return NativeMethods.CM_Get_Class_PropertyW(class_guid, key, out type, buffer,
            ref length, flags).ToNtStatus().CreateResult(throw_on_error, () => buffer.Detach());
    }

    private static NtResult<SafeHGlobalBuffer> GetDeviceInterfaceProperty(string device_instance, DEVPROPKEY key, out DevicePropertyType type, bool throw_on_error)
    {
        int length = 0;
        var result = NativeMethods.CM_Get_Device_Interface_PropertyW(device_instance, key, out type, SafeHGlobalBuffer.Null, ref length, 0);
        if (result != CrError.BUFFER_SMALL)
        {
            return result.ToNtStatus().CreateResultFromError<SafeHGlobalBuffer>(throw_on_error);
        }
        ...
        using var buffer = new SafeHGlobalBuffer(length);
        return NativeMethods.CM_Get_Device_Interface_PropertyW(device_instance, key, out type, buffer,
    ...
    {
        List<DeviceTreeNode> nodes = new();
        while (node != 0)
        {
            DeviceTreeNode curr_node = new(node, parent);
            dict[node] = curr_node;
            nodes.Add(curr_node);
            if (NativeMethods.CM_Get_Child(out int child, node, 0) == CrError.SUCCESS)
            {
                curr_node.AddRange(BuildDeviceTreeNode(curr_node, child, dict));
            }
            if (NativeMethods.CM_Get_Sibling(out node, node, 0) != CrError.SUCCESS)
                break;
        }
        return nodes;
    }
