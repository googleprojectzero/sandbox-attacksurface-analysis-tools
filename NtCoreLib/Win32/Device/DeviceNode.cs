...
public class DeviceNode : IDevicePropertyProvider
{
    ...
    private readonly Lazy<ServiceConfig> _service_info;
    ...
    {
        return ServiceUtils.GetServiceConfiguration(null, Service, 
            false).GetResultOrDefault(new ServiceConfig(null, Service));
    }

    private DeviceNode GetParent()
    {
        if (NativeMethods.CM_Get_Parent(out int parent, _devinst, 0) != CrError.SUCCESS)
            return null;
    ...
    private IReadOnlyList<DeviceStackEntry> BuildDeviceStack()
    {
        ...
        var parent = Parent;
        HashSet<string> upper_filters = new(UpperFilters.Concat(setup_class.UpperFilters), StringComparer.OrdinalIgnoreCase);
        HashSet<string> lower_filters = new(LowerFilters.Concat(setup_class.LowerFilters), StringComparer.OrdinalIgnoreCase);
        ...
        List<DeviceStackEntry> stack = new();
    ...
    internal DeviceNode(int devinst)
    {
        ...
        _service_info = new Lazy<ServiceConfig>(GetServiceConfiguration);
    }
}
